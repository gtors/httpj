HTTPJ allows you to register "event hooks" with the client, that are called
...
client = httpj.Client(event_hooks={'request': [log_request], 'response': [log_response]})
```

You can also use these hooks to install response processing code, such as this
example, which creates a client instance that always raises `httpj.HTTPStatusError`
...
client = httpj.Client(event_hooks={'response': [raise_on_4xx_5xx]})
```
...
client = httpj.Client(event_hooks={'request': [add_timestamp]})
```
...
!!! note
    If you are using HTTPJ's async support, then you need to be aware that
    hooks registered with `httpj.AsyncClient` MUST be async functions,
