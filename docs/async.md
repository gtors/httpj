...
HTTPJ offers a standard synchronous API by default, but also gives you
...
Use `async with httpj.AsyncClient()` if you want a context-managed client...
...
When instantiating a transport instance directly, you need to use `httpj.AsyncHTTPTransport`.

For instance:

```pycon
>>> import httpj
>>> transport = httpj.AsyncHTTPTransport(retries=1)
...
HTTPJ supports either `asyncio` or `trio` as an async environment.
...
For details on calling directly into ASGI applications, see [the `ASGITransport` docs](advanced/transports.md#asgitransport).
